...
export const Filter = ({
	...
	// Получаем уникальные бренды по их именам
	const uniqueBrand = brands
		? [...new Set(brands.map((item) => item.name))]
		: [];
	...
	const handleApplyFilters = () => {
		...
		console.log(checkedTypes, checkedBrands, minPrice, maxPrice);
		setModal(false);
	};
	...
	return (
		<div className="bg-white p-8 rounded-[20px] border-white py-[30px] flex flex-col justify-center max-w-[290px] lg:border-[2px] xl:border-greyDark">
			...
			<div>
				{AnimalTypes &&
					AnimalTypes.map(
						(animal: { animal_type_id: number; name: string }) => (
							<div
								key={animal.animal_type_id}
								className="flex flex-row m-[6px] cursor-pointer"
								onClick={() => toggleTypeCheck(animal.name)}
							>
								{checkedTypes.includes(animal.name) ? (
									<SquareCheck color="#F9A779" className="cursor-pointer" />
								) : (
									<Square className="stroke-1" />
								)}
								<p className="ml-2">{animal.name}</p>
							</div>
						)
					)}
			</div>
			...
			<Button
				className="my-5 h-[40px] text-l px-[7px] py-[7px] xl:rounded-[12px] xl:px-[10px] xl:py-[10px] xl:text-base"
			...
			{/* <Button
				variant="OUTLINE"
				className="my-1 h-[40px] text-l px-[7px] py-[7px] xl:rounded-[12px] xl:px-[10px] xl:py-[10px] xl:text-base"
			>
				Сбросить
			</Button> */}
		</div>
